...
export AbstractPhysiCellSequence, PhysiCellSnapshot, PhysiCellSequence,
       AgentID, AgentDict,
       cellLabels, cellTypeToNameDict, substrateNames,
       loadCells!, loadSubstrates!, loadMesh!, loadGraph!,
       pathToOutputFileBase, pathToOutputXML,
       cellDataSequence, getCellDataSequence
...
A single snapshot of a PhysiCell simulation, identified by the path to its output folder and a snapshot index.
...
# Constructor
- `PhysiCellSnapshot(folder::AbstractString, index::Union{Integer,Symbol}, labels::Vector{String}=String[], substrate_names::Vector{String}=String[]; kwargs...)`

# Fields
- `folder::String`: The path to the PhysiCell output folder.
...
# Optional Arguments
- `labels::Vector{String}=String[]`: A vector of cell data labels. If empty, they are read from the snapshot XML.
- `substrate_names::Vector{String}=String[]`: A vector of substrate names. If empty, they are read from the snapshot XML.
...
snapshot = PhysiCellSnapshot(folder, index)

# :initial or :final are the accepted symbols
snapshot = PhysiCellSnapshot(folder, :initial)

# Load with specific data types
snapshot = PhysiCellSnapshot(folder, index; include_cells=true, include_substrates=true)
```
"""
...
function PhysiCellSnapshot(folder::AbstractString, index::Union{Integer, Symbol},
    ...
    filepath_base = pathToOutputFileBase(folder, index)
    path_to_xml = "$(filepath_base).xml"
    ...
    if include_cells
        if _loadCells!(cells, filepath_base, cell_type_to_name_dict, labels) |> ismissing
            println("Could not load cell data for snapshot $(index) in $folder. Returning missing.")
            return missing
        end
    end
    substrates = DataFrame()
    if include_substrates
        if _loadSubstrates!(substrates, filepath_base, substrate_names) |> ismissing
            println("Could not load substrate data for snapshot $(index) in $folder. Returning missing.")
            return missing
        end
    end
    mesh = Dict{String, Vector{Float64}}()
    ...
    if include_attachments
        if _loadGraph!(attachments, "$(filepath_base)_attached_cells_graph.txt") |> ismissing
            println("Could not load attachments for snapshot $(index) in $folder. Returning missing.")
            return missing
        end
    end
    ...
    if include_spring_attachments
        if _loadGraph!(spring_attachments, "$(filepath_base)_spring_attached_cells_graph.txt") |> ismissing
            println("Could not load spring attachments for snapshot $(index) in $folder. Returning missing.")
            return missing
        end
    end
    ...
    if include_neighbors
        if _loadGraph!(neighbors, "$(filepath_base)_cell_neighbor_graph.txt") |> ismissing
            println("Could not load neighbors for snapshot $(index) in $folder. Returning missing.")
            return missing
        end
    end
    free(xml_doc)
    return PhysiCellSnapshot(String(folder), index, time, runtime, DataFrame(cells), substrates, mesh, attachments, spring_attachments, neighbors)
end

function Base.show(io::IO, snapshot::PhysiCellSnapshot)
    println(io, "PhysiCellSnapshot (Folder=$(snapshot.folder), Index=$(snapshot.index))")
...
Convert an index to the output filename stem (no extension) in the output folder.

The index can be an integer or a symbol (`:initial` or `:final`).

```jldoctest
julia> PhysiCellOutput.indexToFilename(0)
"output00000000"
```
```jldoctest
julia> PhysiCellOutput.indexToFilename(:initial)
"initial"
```
"""
...
indexToFilename(index::Integer) = "output$(lpad(index,8,"0"))"
...
A sequence of PhysiCell snapshots loaded from a single output folder.

By default, only the folder, index, and time are recorded for each `PhysiCellSnapshot` in the sequence.
...
# Constructor
- `PhysiCellSequence(folder::AbstractString; kwargs...)`
...
```julia
folder = "/path/to/PhysiCell/output"
sequence = PhysiCellSequence(folder; include_cells=true, include_substrates=true) # loads cell and substrate data
sequence = PhysiCellSequence(folder; include_attachments=true, include_spring_attachments=true) # loads attachment data
sequence = PhysiCellSequence(folder; include_mesh=true, include_neighbors=true) # loads mesh and neighbor data
```
"""
...
function PhysiCellSequence(folder::AbstractString;
    include_cells::Bool=false,
    include_substrates::Bool=false,
    include_mesh::Bool=false,
    include_attachments::Bool=false,
    ...
    path_to_xml = pathToOutputXML(folder, :initial)
    ...
    index_to_snapshot = index -> PhysiCellSnapshot(folder, index, labels, substrate_names;
    ...
    index = 0
    while isfile(pathToOutputXML(folder, index))
        push!(snapshots, index_to_snapshot(index))
        index += 1
    end
    return PhysiCellSequence(String(folder), snapshots, cell_type_to_name_dict, labels, substrate_names)
end

function Base.show(io::IO, sequence::PhysiCellSequence)
    println(io, "PhysiCellSequence (Folder=$(sequence.folder))")
...
"""
    pathToOutputFileBase(folder::AbstractString, index::Union{Integer,Symbol})

Return the path to the output files for a snapshot of a PhysiCell simulation, i.e., everything but the file extension.
"""
pathToOutputFileBase(folder::AbstractString, index::Union{Integer,Symbol}) = joinpath(folder, indexToFilename(index))
...
Return the path to the output files for a snapshot of a PhysiCell simulation, i.e., everything but the file extension.
"""
pathToOutputFileBase(snapshot::PhysiCellSnapshot) = pathToOutputFileBase(snapshot.folder, snapshot.index)

"""
    pathToOutputXML(folder::AbstractString, index::Union{Integer,Symbol}=:initial)

Return the path to the XML output file for a snapshot of a PhysiCell simulation.
"""
pathToOutputXML(folder::AbstractString, index::Union{Integer,Symbol}=:initial) = "$(pathToOutputFileBase(folder, index)).xml"
...
Return the path to the XML output file for a snapshot of a PhysiCell simulation.
"""
pathToOutputXML(snapshot::PhysiCellSnapshot) = pathToOutputXML(snapshot.folder, snapshot.index)

"""
    cellLabels(path_to_file::String)

Return the labels from the XML file for a PhysiCell simulation, i.e., the names of the cell data fields.

# Arguments
- `path_to_file`: The path to the snapshot XML file. Can also use an `XMLDocument`, a [`PhysiCellSnapshot`](@ref), or a [`PhysiCellSequence`](@ref).
"""
...
"""
    cellTypeToNameDict(path_to_file::String)
...
"""
    substrateNames(path_to_file::String)
...
# Examples
```julia
sequence = PhysiCellSequence(folder) # does not load cell data without setting `PhysiCellSequence(folder; include_cells=true)`
loadCells!(sequence)
```
"""
...
"""
    _loadSubstrates!(substrates::DataFrame, filepath_base::String, substrate_names::Vector{String})
...
"""
    cellDataSequence(folder::AbstractString, labels::Vector{String}; include_dead::Bool=false, include_cell_type_name::Bool=false)
...
# Arguments
- `folder::AbstractString`: The path to the PhysiCell output folder. Alternatively, can be a [`PhysiCellSequence`](@ref) object.
...
# Examples
```julia
data = cellDataSequence(folder, ["position", "elapsed_time_in_phase"]; include_dead=true, include_cell_type_name=true)
...
```
"""
function cellDataSequence(folder::AbstractString, labels::Vector{String}; kwargs...)
    sequence = PhysiCellSequence(folder)
    return cellDataSequence(sequence, labels; kwargs...)
end

function cellDataSequence(folder::AbstractString, label::String; kwargs...)
    return cellDataSequence(folder, [label]; kwargs...)
end
